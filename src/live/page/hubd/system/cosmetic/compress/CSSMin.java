...
public class CSSMin {
    ...
    /**
     * Instantiates a new Css min.
     *
     * @param in the in
     * @throws IOException the io exception
     */
    private CSSMin(Reader in) throws IOException {
        // Read the stream...
        int c;
        while ((c = in.read()) != -1) {
            srcsb.append((char) c);
    ...
    public static String minify(String css) {
        try (StringReader reader = new StringReader(css); StringWriter str = new StringWriter()) {
            new CSSMin(reader).minify(str, Integer.MAX_VALUE);
            return str.toString();
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     *
     * @param out          the out
     * @param linebreakpos the linebreakpos
     * @throws IOException the io exception
     */
    public void minify(Writer out, int linebreakpos)
            throws IOException {

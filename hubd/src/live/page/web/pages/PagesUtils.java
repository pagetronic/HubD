...
public class PagesUtils {
	...
	public static Json revision(String _id) {
		...
		Json revision = Db.aggregate("Revisions",
				Arrays.asList(
						...
						Aggregates.lookup("Pages", "childrens", "_id", "childrens"),
	...
	public static Json revision(String url, String domain, String _id) {
		...
		Json page = PagesAggregator.getPage(_id != null ?
				...
				Filters.and(Filters.eq("url", urls[urls.length - 1]), Filters.eq("lng", Settings.getLang(domain))), null, true);
		...
		List<Json> prev_childrens = revision.getListJson("childrens");
		List<Json> prev_docs = revision.getListJson("docs");
		revision.remove("childrens").remove("docs");
		...
		List<Json> childrens = page.getListJson("childrens");
		List<String> childrens_sort = new ArrayList<>();
		List<Object> childrens_sort_ = revision.getList("childrens", Object.class);
		if (childrens_sort_ != null) {
			childrens_sort_.forEach(children -> {
				childrens_sort.add(children.getClass().isAssignableFrom(String.class) ? children.toString() : ((Json) children).getId());
			});
		}
		if (childrens != null && childrens_sort != null) {
			List<Json> real_childrens = new ArrayList<>();
			for (String chidren_id : childrens_sort) {
				finder:
				for (Json children : childrens) {
					if (chidren_id.equals(children.getId())) {
						real_childrens.add(children);
			...
			if (childrens.size() > 0) {
				real_childrens.addAll(childrens);
			}
			revision.put("childrens", real_childrens);
		} else {
			revision.put("childrens", childrens);
		}

		if (revision.get("childrens") == null) {
			revision.put("childrens", prev_childrens);
		}
	...
	public static Json draft(Json data, Users user) {
		...
		if (!data.getString("revision", "").equals("")) {
	...
	public static Json save(Json data, Users user) {
		...
		List<String> childrens = data.getList("childrens");
		if (childrens != null && !data.getString("origine", "").equals("") && childrens.contains(data.getString("origine"))) {
			errors.add(new Json("element", "childrens").put("message", "SAME_ID"));
		}

		List<String> parents = data.getList("parents");
		if (parents != null) {
			if (!data.getString("origine", "").equals("") && parents.contains(data.getString("origine"))) {
		...
		if (page == null) {
			...
			if (!data.getString("origine", "").equals("")) {
		...
		if (restore || (childrens != null && !childrens.equals(page.getList("childrens")))) {
			revision.put("childrens", childrens);
			page.put("childrens", childrens);
		}
		...
		if (childrens != null) {
			for (String children_id : childrens) {
				Json children = Db.findOneAndUpdate("Pages", Filters.and(Filters.eq("_id", children_id), Filters.ne("parents", page.getId())), new Json("$push", new Json("parents", page.getId())));
				if (children != null) {
	...
	public static Json sortChildrens(String id, List<String> childrens, Users user) {
		Json page = Db.findOneAndUpdate("Pages", Filters.eq("_id", id), Updates.set("childrens", childrens), new FindOneAndUpdateOptions().returnDocument(ReturnDocument.AFTER));
		if (page != null) {
			Db.save("Revisions", new Json("edit", new Date()).put("childrens", page.get("childrens")).put("editor", user.getId()).put("origine", id));
			return new Json("ok", true);
		}
		return new Json("ok", false);
	}
	...
	public static Json childrensPage(String parent) {
		...
		if (parent.equals("ROOT")) {
			...
			pipeline.add(Aggregates.project(new Json().put("childrens_", new Json("$cond", Arrays.asList(new Json("$eq", Arrays.asList("$childrens", new BsonUndefined())),
					new ArrayList<>(),
					"$childrens")
			))));
			...
			pipeline.add(Aggregates.project(new Json().put("_id", false).put("id", "$_id").put("childrens_", true)
					.put("childrens", new Json("id", "$childrens._id").put("title", true).put("order", new Json("$indexOfArray", Arrays.asList("$childrens_", "$childrens._id"))))
			));
			pipeline.add(Aggregates.replaceRoot("$childrens"));
		}
		pipeline.add(Aggregates.lookup("Pages", "id", "parents", "childrens"));
